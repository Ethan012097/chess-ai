...
| **P21** | 連續 5 代自我對弈（第一輪，200 局配方） | ✅ 跑完，**零通過** —— 但原因是把關沒解析度 |
| **P21b** | 第 6–8 代（500 局配方 + elo1=50） | ✅ **通過兩次，`best.pt` 換掉兩次** |

**198 條測試全過**（`python -m pytest tests/ -q`），全部在 CPU 上跑，不需要 GPU 或 checkpoint。

### P21b 的結果（2026-08-16，目前最新）

改了兩件事就從「零通過」變成「三代通過兩代」：
**每代 500 局（原 200）、訓練 800 步（原 1500）、`sprt_elo1` 50（原 20）。**

| 代 | 盤面 | 平均步數 | 和局 | value_loss | Elo（vs 當時的 best） | SPRT | best.pt |
|---|---|---|---|---|---|---|---|
| 6 | 43,925 | 87.8 | 28.2 % | 0.417 | **+90.7** ± 66.6 | llr 3.04 → **H1** | ✅ 更新 |
| 7 | 45,511 | 91.0 | 22.2 % | 0.419 | +2.9 | `?` | 保留 |
| 8 | 44,646 | 89.3 | 20.4 % | 0.415 | **+90.8** ± 66.2 | llr 3.07 → **H1** | ✅ 更新 |

- 第 6 代從 `best.pt` 乾淨開始；第 7、8 代接續候選模型。
  **第 7 代持平（不是崩掉）、第 8 代再跳一級** —— 跟第一輪第 4→5 代掉 67 Elo 的行為明顯不同。
- 兩次通過都是 llr 衝破 3.0 才收，不是壓線。
- 和局比例一路降（28.2 → 22.2 → 20.4 %），模型愈來愈敢下決定性的棋。
- SPRT 的 PGN 裡各有 1 局 `Result "*"` / `Termination "unterminated"`，
  那是**判定結論時被中止的最後一局**（`GameDuration 00:00:00`），
  不是 §5-1 的 `bestmove 0000` 問題 —— 已核對過。

**還沒量到的數字**：那兩個 +90 Elo 都是對「當時的 best」量的，串起來約 +180，
但那是兩個各 ±66 的估計相加。要知道真正的收穫，得讓現在的 `best.pt` 直接對
`best_supervised.pt` 打一場（兩個檔案都在），約 20–30 分鐘。**在那之前不要引用 +180。**

### P21 第一輪的結果（保留，因為教訓比數字重要）
...
1. **這個 SPRT 設定沒有解析度，三個 `?` 幾乎不帶資訊。**
   ...
   **P21b 證實了這一點**：同樣的局數，把 `elo1` 改成 50 之後兩代都判得出 H1。
   量不出來的時候調參數只是在猜 —— 先修尺。
...
| 檔案 | 內容 | 說明 |
|---|---|---|
| `models/best.pt` | small，C96×8，1.54 M 參數 | **目前使用中**。已經**不是**監督式那一版 —— 8/16 被第 6 代與第 8 代各覆蓋一次（兩次都過 SPRT） |
| `models/best_supervised.pt` | 監督式的原始版本 | **不要動它**：現在是唯一的對照基準，也在 GitHub v1.0 Release |
| `models/selfplay_candidate.pt` | 持續訓練中的候選（第 8 代） | 每代都在它上面繼續練，跟 best.pt 分開（§2.3） |
| `models/base/best.pt` | base，C128×10，3.19 M 參數 | 訓練完成但 **SPRT 未通過，未採用** |
| `models/epoch_*.pt` | small 的逐 epoch checkpoint | 保留供比較 |

### 關鍵實測數字

⚠️ 以下全部是**監督式模型**（= 現在的 `best_supervised.pt`）的數字。
`best.pt` 已經往前走了兩代，這些數字還沒有重測。
...
1. **先量真正的收穫**：現在的 `best.pt` 直接對 `best_supervised.pt` 跑一次 SPRT
   （約 20–30 分鐘）。串接起來的 +180 Elo 是兩個 ±66 估計相加，不能直接拿來引用。
   量完再回頭更新「關鍵實測數字」那一區。
2. **繼續跑更多代**：配方已經驗證有效，重下同一道指令即可
   （`-Iterations N -Games 500 -TrainSteps 800`）。每代約 5 小時。
3. **讓認輸機制真的會觸發**（七代都是 0.0 %）：−0.9 連 10 步從沒觸發過，
   每代仍在白算必敗殘局；500 局的規模下這筆浪費比先前更大。
4. 若要量絕對棋力：`--mode match --uci-elo 1320 1400 1500`（已實作，需 150+ 局才有意義）
5. Phase 1.5 備案（用 Stockfish 評分重訓 value）目前**不需要** —— Spearman 0.7642 已超過 0.75 的驗收目標
