...
### `base` preset 的實測（3,192,026 參數、8 epoch、batch 512）

同一批資料、同樣的超參數，只換 preset：

| epoch | policy top-1 | policy top-5 | policy loss | value MAE |
|---|---|---|---|---|
| 1 | 47.35 % | 86.27 % | 1.6530 | 0.7457 |
| 2 | 49.53 % | 88.01 % | 1.5637 | 0.7408 |
| 4 | 51.51 % | 89.25 % | 1.4901 | 0.7344 |
| 6 | 52.30 % | 89.62 % | 1.4645 | 0.7226 |
| **8** | **52.46 %** | **89.64 %** | **1.4655** | **0.7198** |

與 `small` 並列（都是各自的最終成績）：

| | `small`（C96×8，12 epoch） | `base`（C128×10，8 epoch） |
|---|---|---|
| 參數量 | 1,541,434 | **3,192,026**（×2.07） |
| val top-1 | 51.60 % | **52.54 %** |
| val top-5 | 89.13 % | **89.64 %** |
| value MAE | 0.7253 | **0.7198** |
| 前向速度 | 2.71 ms/batch32 | 3.16 ms（慢 17 %） |
| **SPRT 對打** | — | **+12.6 ± 21.3 Elo（800 局，無結論）** |

**三個驗證指標 base 全面領先，但棋力上分不出來。** 這正是為什麼不能只看
val top-1 就換模型。而且**多打反而更沒把握**：

| 局數 | 比分 | Elo | llr（門檻 2.94） |
|---|---|---|---|
| 300 | 127-103-70 | +27.9 ± 34.6 | 1.15（39 %） |
| **800** | **328-299-173** | **+12.6 ± 21.3** | **0.437（15 %）** |

llr 往回走是有意義的訊號，不是雜訊：H1 的假設是「強 20 Elo 以上」，
而點估計從 +27.9 縮到 +12.6，**低於門檻**，所以證據其實在累積反對 H1。
300 局那個 +27.9 是小樣本的高估。

**最該記住的一課：參數量加倍只換到最多十幾 Elo。** 而且慢 17 % 這件事在這裡
幾乎沒有代價（MCTS 有九成時間卡在 Python 的著法產生，GPU 只佔 25/143 ms，
換算下來只慢約 3 %）—— 也就是說 base 沒有明顯的缺點，它就只是**沒有明顯變強**。

這指向一個結論：**瓶頸不在模型容量，在資料與標籤。** 想再進一步，
與其加大網路，不如加資料量，或換更乾淨的 value 標籤（見 §7 Phase 1.5）。

`base` 也**沒有**因此取代 `models/best.pt` —— 規則是「SPRT 判定 H1 才換」，
沒判定就不換。想自己重跑：

```powershell
python -m src.evaluate --mode sprt --new models/base/best.pt --old models/best.pt --rounds 800
```

硬體注意：`base` + `batch_size 512` 在 RTX 3060 Laptop 6 GB 上全程沒有 OOM，
約 9,000 盤面/秒，8 個 epoch 約 2.9 小時。訓練時**務必加 `--model-dir`**：

```powershell
python -m src.train --preset base --batch-size 512 --epochs 8 --model-dir models/base
```

不加的話 `epoch_N.pt` 會蓋掉舊 preset 的同名檔，而且 `best.pt` 會依 val top-1
自動覆寫 —— **那是完全沒有經過對局把關的**，正是這一節要避免的事。
...
> 依規格 §10，如果 MCTS 做完只贏 greedy 五六十 Elo（而不是預期的 200+），
...
```
Score of MCTS800 vs greedy: 27 - 0 - 3  [0.950] 30
SPRT: llr 3.03, H1 was accepted
```

**27 勝 0 敗 3 和，30 局就達到統計顯著。** 對照修正 §13 兩個 MCTS bug 之前：

| | 比分 | 得分率 | Elo（95 % Wilson 區間） | 達顯著所需局數 |
|---|---|---|---|---|
| 修正前 | 64-16-7 | 77.6 % | +216 `[+129, +302]` | 87 |
| **修正後** | **27-0-3** | **95.0 %** | **+512 `[+251, +772]`** | **30** |

**那兩個 bug 大約讓 MCTS 少了兩百多 Elo。** 注意兩個區間在 `[+251, +302]`
有小幅重疊，所以嚴格說「提升了 296 Elo」是過度解讀；能斷言的是
**修正後至少強 251 Elo，而修正前的上界只有 302**，兩者幾乎不相容。

最值得記住的是：**修正前的表面指標完全正常** —— SPRT 照樣判定 H1、
勝率 77.6 %、規格要求的「強 200 Elo 以上」也達標。
如果沒有去看謎題對照裡的訪問次數分佈，這兩個 bug 會一直留在裡面。

**謎題測驗（同一批題目，McNemar 配對檢定，各 28 題/桶）**

| Rating | greedy | MCTS400 | b | c | p 值 | 判定 |
|---|---|---|---|---|---|---|
| `<1000` | 96.4 % | **100.0 %** | 0 | 1 | 1.000 | 不顯著 |
| `1000-1200` | 78.6 % | **96.4 %** | 0 | 5 | 0.063 | 邊緣（MCTS 較好） |
| `1200-1400` | 85.7 % | 85.7 % | 1 | 1 | 1.000 | 不顯著 |
| `1400-1600` | 60.7 % | 64.3 % | 3 | 4 | 1.000 | 不顯著 |
| `1600-1800` | 71.4 % | 82.1 % | 2 | 5 | 0.453 | 不顯著 |
| `1800-2000` | 42.9 % | **67.9 %** | 1 | 8 | **0.039** | **顯著（MCTS 較好）** |
| `2000+` | 46.4 % | 60.7 % | 2 | 6 | 0.289 | 不顯著 |

`b` = greedy 對而 MCTS 錯的題數，`c` = 反過來。**只有這些不一致對帶訊息**
—— 兩邊都對或都錯的題目不影響檢定。

**為什麼改用 McNemar**：兩個搜尋器跑的是同一批題目，那是配對資料。
之前用獨立樣本的標準誤（每桶 ±5 %）判斷顯著性是錯的方法，它會把真實的差異
埋進雜訊裡。配對檢定只看不一致對，靈敏度高得多：`1800-2000` 桶的 8 比 1
在配對檢定下 p = 0.039，用獨立樣本看則只是「25 個百分點，可能是雜訊」。

**MCTS 現在在每一個分桶都持平或更好。** 這是修掉兩個 bug 之後的結果，見下一節。

### 兩個讓 MCTS 在簡單局面反而變差的 bug（已修）

第一版的對照表裡，`<1000` 桶 greedy 89 % 而 MCTS400 只有 82 % —— 那些題目多半是
一步將死或吃免費子，400 次模擬理應找得更準。用 `--compare-with` 印出
「greedy 對而 MCTS 錯」的題目細節之後，訪問次數分佈直接指出問題：

```
28 個合法著法，訪問次數分佈 = {32次: 12個, 0次: 15個, 16次: 1個}
其中正解 Rd8+ 的 prior 是 0.9270，卻跟 prior 0.0016 的著法拿到一樣多的訪問
```

「剛好等於 batch size 的整齊倍數」是決定性線索。兩個獨立的錯誤：

**一、virtual loss 的符號寫反了。** `value_sum` 的約定是「該節點走棋方的視角」，
而父節點用 `-child.q()` 評估它。要讓剛選過的路徑變得沒吸引力，
必須讓 `value_sum` **增加**；原本寫成減，效果完全相反 ——
剛選過的那條變成 PUCT 分數最高的，整批 32 個葉節點全擠到同一個子節點。
`400 = 12×32 + 16` 這個算式把症狀解釋得一分不差。

**二、未訪問節點的 Q 用 0，等於病態的樂觀初始化。** 在劣勢局面裡
已探索子節點的 `-Q(父視角)` 約 −0.9，而未探索的是 0 —— 每個沒走過的著法
都比所有走過的好，搜尋於是一路往外攤平。改用 **FPU（First Play Urgency）**：
未訪問節點的預設值 = 父節點的 Q 減 0.25。

順帶還發現 virtual loss **不能套在根節點上**：根節點的 Q 正是 FPU 的基準，
被灌水之後未訪問子節點的預設值會虛高，又會退回攤平。

修正後同一個局面：正解拿到 **101 次訪問，其餘每個 1 次**（修正前全部 32 次）。

第三項是你原本懷疑的：**MCTS 沒有繼承 greedy 的一步將死檢查**。這確實存在，
但它造成的「答錯」多半是假的 —— 有些局面存在兩個一步殺，兩邊挑到不同的那一個，
對照時看起來像有一邊錯了，其實都將死了。現在兩個搜尋器共用同一支
`find_mate_in_one()`，選擇一致，而且有立即將死就不必搜尋。

`tests/test_mcts.py` 補了六條測試把這三件事釘住，其中一條會在 virtual loss
符號寫反時立刻失敗。
...
用 `scripts/check_opening_value.py` 對 60 條開局書逐 ply 量測，跟 Stockfish depth 12 對照
（都是白方視角的 centipawn）：

| ply | 模型 | Stockfish | 偏差 | 在訓練集裡嗎 |
|---|---|---|---|---|
| **0** | **+249** | +39 | **+210** | 否 |
| 1 | −26 | +31 | −58 | 否 |
| 2–7 | — | — | 平均 **−17** | 否 |
...
`preprocess.py` 的取樣是 `range(skip_opening_plies, ...)`，預設 8，
所以 **ply 0–7 從來沒有進過訓練集**。這給了一條很乾淨的分界線，而結果排除了兩種解釋：

- **不是「記憶訓練資料」**：起始盤面根本沒進過訓練集，而且 elite 棋局的白方期望得分
  只有約 +0.10，量級也對不上 +2.49。
- **不是「分佈外外插」**：ply 1–7 跟起始盤面一樣從未出現在訓練集，偏差卻只有 −17 cp。
  「分佈外」作為一個範疇解釋不了為什麼只有其中一個盤面壞掉。

**異常侷限在起始盤面附近，而且它是自我矛盾的。** 三個補充量測：

```
標準起始盤面（白走）        value = +0.458  → +249 cp
同一盤面但輪黑走            value = +0.458  → +249 cp   ← 完全相同
起始盤面但拿掉易位權        value = +0.063  →  +28 cp   ← 偏差幾乎消失
1.a3 之後                   value = +0.287  → +138 cp
1.e4 之後                   value = +0.014  →   +6 cp
```

第二行是關鍵：canonical 編碼讓「白方走的起始盤面」與「黑方走的起始盤面」變成
**同一個輸入張量**，所以網路必然給出同一個數字。它等於在說「不管輪到誰，
走的那一方領先 2.49 兵」—— 對稱盤面上唯一自洽的答案只能是 0。

第三行指出觸發條件與**易位權平面**高度相關：四個易位權全開 + 32 子完整 + halfmove 0
這個組合在訓練集裡只會出現在很安靜的開局，而那類局面白方得分確實偏高；
把易位權清掉，偏差就從 +249 掉到 +28。

**實際影響很小。** 它只發生在第一步的根節點，而 MCTS 往前看一層就修正掉了
（3200 次模擬後 `root.q()` 回到 +0.014）。從 ply 1 開始 value head 的偏差就在 ±60 cp 內，
所以 MCTS 在整個開局階段的葉節點評估是可信的 —— 這點原本有疑慮，量過之後可以排除。

驗收：

```powershell
python scripts/check_opening_value.py --lines 60 --depth 12
```
...
只有 §11 的 MCTS vs greedy 需要重跑。那次 107 局只有 1 局和棋，
...
## 14. Phase 1.5 備案：更好的 value 標籤（目前不實作）
...
規格與最新進度見 **`CLAUDE.md`**（Phase 1 與 Phase 2 已整合成一份）。

Phase 1 的 M0–M10 與 Phase 2 的 P1–P20 全部完成，P21（連續 5 代自我對弈）執行中。
`config.yaml` 的 `mcts:` 與 `selfplay:` 區塊現在都真的會被讀取。
`data/selfplay/` 跟 `data/` 其餘部分一樣不進 git。
